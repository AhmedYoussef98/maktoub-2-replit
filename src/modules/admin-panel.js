    ...
    const tableHTML = `
      <div class="table-wrapper">
        <table>
          ...
          <tbody>
            ${users.map(user => `
              <tr data-user-email="${Utils.escapeHtml(user.email)}">
                <td>
                  <i class="fas fa-user" style="color: #999; margin-left: 8px;"></i>
                  ${Utils.escapeHtml(user.email)}
                </td>
                <td>${Utils.escapeHtml(user.full_name || '-')}</td>
                <td>${Utils.escapeHtml(user.phone_number || '-')}</td>
                <td>
                  <span class="badge badge-${user.role || 'user'}">
                    ${user.role === 'admin' ? '<i class="fas fa-shield-alt"></i>' : ''}
                    ${user.role === 'admin' ? 'مسؤول' : 'مستخدم'}
                  </span>
                </td>
                <td>
                  <span class="badge badge-${user.status || 'inactive'}">
                ...
                <td>
                  <button
                    ...
                    onclick="AdminPanel.showEditModal('${Utils.escapeHtml(user.email)}')"
                    title="تعديل">
                    <i class="fas fa-edit"></i>
                  </button>
                  <button
                    class="btn btn-danger"
                    onclick="AdminPanel.handleDeleteUser('${Utils.escapeHtml(user.email)}')"
                    title="حذف">
